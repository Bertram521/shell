...
then
     ...
     cd $BACKUP_DIR/$DATE
for table in $(mysql -u${DB_USER} -p${DB_PASSWD} -h10.161.28.44  -e "show tables from jescard_data;" | egrep -w '(BrandR|Brand|Category|CategoryR|IgnoredEntitySource|Mall|RelevanceSession|SkuGood|SpuGood|LowestSku|HightestSku|GoodsLanguage|BrandLanguage)')
do
